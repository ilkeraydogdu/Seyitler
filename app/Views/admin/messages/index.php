                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors <?= empty($m['is_read']) ? 'bg-emerald-50/30' : '' ?> group">
                                ...
                                <td class="py-4 px-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="<?= url('/podmin/messages/' . $m['id']) ?>" class="px-3.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded-xl text-xs font-bold transition-all border border-emerald-200/60 shadow-2xs">
                                            İncele
                                        </a>
                                        <form action="<?= url('/podmin/messages/delete/' . $m['id']) ?>" method="POST" onsubmit="return confirm('Bu mesajı silmek istediğinize emin misiniz?');" class="inline">
                                            <?= csrf_field() ?>
                                            <button type="submit" class="p-1.5 text-slate-400 hover:text-rose-600 rounded-xl hover:bg-rose-50 transition-colors cursor-pointer" title="Sil">
